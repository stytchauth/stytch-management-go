...
// DFPPASettings returns a list of all possible DFPPA settings.
...
// DFPPAOnChallengeAction is a type specifying the action to take when a DFPPA "challenge" verdict
// is returned.
type DFPPAOnChallengeAction string

const (
	// DFPPAOnChallengeActionAllow is the action to allow the request when a "challenge" verdict is
	// returned.
	DFPPAOnChallengeActionAllow DFPPAOnChallengeAction = "ALLOW"
	// DFPPAOnChallengeActionBlock is the action to block the request when a "challenge" verdict is
	// returned.
	DFPPAOnChallengeActionBlock DFPPAOnChallengeAction = "BLOCK"
	// DFPPAOnChallengeActionTriggerCaptcha is the action to trigger a CAPTCHA when a "challenge"
	// verdict is returned.
	DFPPAOnChallengeActionTriggerCaptcha DFPPAOnChallengeAction = "TRIGGER_CAPTCHA"
)

// DFPPAOnChallengeActions returns a list of all possible DFPPA on-challenge actions.
...
// AuthorizedB2BDomain is a type specifying the domain and slug pattern authorized for use in the
// SDK for a B2B project.
type AuthorizedB2BDomain struct {
	// Domain is the domain name. Stytch uses the same-origin policy to determine matches.
	Domain string `json:"domain,omitempty"`
	// SlugPattern is the slug pattern that can be used to support authentication flows specific to
	// each organization. An example value here might be 'https://{{slug}}.example.com'. The value
	// **must** include '{{slug}}' as a placeholder for the slug.
...
type SMSAutofillMetadata struct {
	...
	// MetadataValue is the value of the metadata to use for autofill. This should be the associated
	// domain name (for MetadataType "domain") or application hash (for MetadataType "hash").
...
const (
	...
	// HttpOnlyCookiesSettingEnabled indicates that cookies should be set with the HttpOnly flag when
	// using a custom authentication domain and not set otherwise.
	HttpOnlyCookiesSettingEnabled HttpOnlyCookiesSetting = "ENABLED"
	// HttpOnlyCookiesSettingEnforced indicates that cookies should be set with the HttpOnly flag when
	// using a custom authentication domain, and requests to other domains should be rejected.
	HttpOnlyCookiesSettingEnforced HttpOnlyCookiesSetting = "ENFORCED"
)

// HttpOnlyCookiesSettings returns a list of all possible HttpOnly cookies settings.
...
type ConsumerBasicConfig struct {
	// Enabled indicates whether the consumer project SDK is enabled. This allows the SDK to manage
...
type ConsumerMagicLinksConfig struct {
	...
	// PKCERequired indicates whether PKCE is required for magic links. PKCE increases security by
	// introducing a one-time secret for each auth flow to ensure the user starts and completes each
	// auth flow from the same application on the device. This prevents a malicious app from
	// intercepting a redirect and authenticating with the user's token. PKCE is enabled by default
	// for mobile SDKs.
	PKCERequired bool `json:"pkce_required"`
}

type ConsumerOTPsConfig struct {
	// SMSLoginOrCreateEnabled indicates whether the SMS OTP login or create endpoint is enabled in
	// the SDK.
	SMSLoginOrCreateEnabled bool `json:"sms_login_or_create_enabled"`
	// WhatsAppLoginOrCreateEnabled indicates whether the WhatsApp OTP login or create endpoint is
	// enabled in the SDK.
	WhatsAppLoginOrCreateEnabled bool `json:"whatsapp_login_or_create_enabled"`
	// EmailLoginOrCreateEnabled indicates whether the email OTP login or create endpoint is enabled
	// in the SDK.
...
type ConsumerOAuthConfig struct {
	...
	Enabled bool `json:"enabled"`
	// PKCERequired indicates whether PKCE is required for OAuth. PKCE increases security by
	// introducing a one-time secret for each auth flow to ensure the user starts and completes each
	// auth flow from the same application on the device. This prevents a malicious app from
	// intercepting a redirect and authenticating with the user's token. PKCE is enabled by default
	// for mobile SDKs.
...
type ConsumerWebAuthnConfig struct {
	...
	Enabled bool `json:"enabled"`
	// CreateWebAuthns indicates whether WebAuthn creation is enabled in the SDK.
...
type ConsumerDFPPAConfig struct {
	// Enabled indicates whether Device Fingerprinting Protected Auth is enabled in the SDK.
...
type ConsumerBiometricsConfig struct {
	// Enabled indicates whether biometrics endpoints are enabled in the SDK.
	Enabled bool `json:"enabled"`
...
type ConsumerPasswordsConfig struct {
	// Enabled indicates whether password endpoints are enabled in the SDK.
	Enabled bool `json:"enabled"`
...
type B2BBasicConfig struct {
	// Enabled indicates whether the B2B project SDK is enabled. This allows the SDK to manage user
	// and session data.
	Enabled bool `json:"enabled"`
...
type B2BSSOConfig struct {
	...
	Enabled bool `json:"enabled"`
	// PKCERequired indicates whether PKCE is required for SSO. PKCE increases security by
	// introducing a one-time secret for each auth flow to ensure the user starts and completes each
	// auth flow from the same application on the device. This prevents a malicious app from
	// intercepting a redirect and authenticating with the user's token. PKCE is enabled by default
	// for mobile SDKs.
...
type GetConsumerConfigRequest struct {
	// ProjectSlug is the slug of the project for which to get the consumer SDK config.
	ProjectSlug string `json:"-"`
	// EnvironmentSlug is the slug of the environment for which to get the consumer SDK config.
...
type SetConsumerConfigRequest struct {
	// ProjectSlug is the slug of the project for which to set the consumer SDK config.
	ProjectSlug string `json:"-"`
	// EnvironmentSlug is the slug of the environment for which to set the consumer SDK config.
	EnvironmentSlug string `json:"-"`

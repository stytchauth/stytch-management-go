...
type PasswordStrengthConfig struct {
	...
	// is nil when using the ZXCVBN validation_policy.
	...
	// Symbols) when using a LUDS validation_policy. This field is nil when using the ZXCVBN validation_policy.
...
type SetRequest struct {
	...
	LudsMinPasswordLength *int `json:"luds_min_password_length,omitempty"`
	// LudsMinPasswordComplexity is the minimum number of "character types" in a password (Lowercase, Uppercase, Digits,
	// Symbols) when using a LUDS validation_policy. This field is ignored when using the ZXCVBN validation_policy.
	// If present, this must be a number in the range [1, 4].
	LudsMinPasswordComplexity *int `json:"luds_min_password_complexity,omitempty"`
}
